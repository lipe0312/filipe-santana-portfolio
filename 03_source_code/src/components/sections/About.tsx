...
const CORE_TECHNOLOGIES = [
  "Python", "C", "C++", "JavaScript", "Java", "React", "TypeScript",
  "Flask", "OpenCV", "YOLO", "ESP32", "MQTT", "Git", "Linux", "SQL",
  "Assembly", "TensorFlow",
];

const SOFT_SKILLS = [
  "Technical Leadership", "Academic Research", "Strategic Thinking", "Sales",
  "Complex Problem Solving", "Hardware-Software Integration",
  "Cross-layer Systems Thinking", "Adaptability",
];
...
const PIXEL_GRID_HIGHLIGHT: CSSProperties = {
  ...
  backgroundSize: "20px 20px",
  maskImage: "radial-gradient(circle 150px at var(--mouse-x, -999px) var(--mouse-y, -999px), black, transparent)",
  WebkitMaskImage: "radial-gradient(circle 150px at var(--mouse-x, -999px) var(--mouse-y, -999px), black, transparent)",
};
...
export default function About() {
  ...
  const { ref: statsRevealRef, isVisible: statsVisible } = useIntersectionObserver();
  const { ref: summaryRef, isVisible: summaryVisible } = useIntersectionObserver();
  const { ref: techRef, isVisible: techVisible } = useIntersectionObserver();
  const { ref: diferencialRef, isVisible: diferencialVisible } = useIntersectionObserver();
  const { ref: beyondRef, isVisible: beyondVisible } = useIntersectionObserver();
    ...
    >
      ...
      <div className="relative max-w-5xl mx-auto">
        ...
        <div
          ...
          id="about-stats"
          className={(statsVisible ? "reveal is-visible" : "reveal") + " grid grid-cols-3 gap-8 mb-16"}
            ...
            return (
              <div key={label} className="text-center">
                <div className="text-[clamp(2rem,4vw,3.5rem)] font-bold font-display text-text-primary leading-none mb-2">
                  <span ref={function(el) { counterRefs.current[i] = el; }}>0</span>
                </div>
        ...
        <div
          ...
          className={(summaryVisible ? "reveal is-visible" : "reveal") + " mb-16"}
        ...
        >
          <div
            ...
            className="relative"
            style={{ "--mouse-x": "-999px", "--mouse-y": "-999px" } as unknown as CSSProperties}
          >
            ...
            <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-6" style={{ zIndex: 1 }}>
                ...
                <div className="flex flex-wrap gap-2">
                  {SOFT_SKILLS.map(function(skill, i) {
                    return (
                      <span
                        key={skill}
                        ref={function(el) { tagRefs.current[CORE_TECHNOLOGIES.length + i] = el; }}
                        className="font-mono text-sm bg-zinc-100 text-zinc-800 px-[10px] py-1"
                        style={TAG_STYLE}
                      >
                        {skill}
        ...
        <div
          ...
          className={(diferencialVisible ? "reveal is-visible" : "reveal") + " mb-16"}
